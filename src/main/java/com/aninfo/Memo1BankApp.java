...
import com.aninfo.response.MessageResponse;
...
public class Memo1BankApp {
    ...
    public ResponseEntity<?> createTransactions(
            @RequestParam Long cbu, @RequestParam Double sum, @RequestParam String type) {
        try {
            return ResponseEntity.ok().body(transactionService.createTransaction(cbu, sum, type));
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(new MessageResponse(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getTransactions(@RequestParam Long cbu) {
        try {
            return ResponseEntity.ok().body(transactionService.getTransactionsByCbu(cbu));
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(new MessageResponse(e.getMessage()));
        }
    }
